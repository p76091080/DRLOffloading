...
for episode in range(MAX_EPISODES):
    ...
    for i in range(T):
        ...
        if T_offload < obs[4]:
            ...
            delay_list.append(T_offload)

            offload += 1
            # print("offload reward", reward)
            # elif actions[0] < 0.5:
        else:
            # loacl
            ...
            # print(obs, s_)
            RL.store_transition(obs, actions, -reward, s_)
            env.reward_list.append(reward)   # cost
            delay_list.append(T_local)
            local += 1
    ...
    print('Episode:', episode, ' Steps: %2d' %
          i, 'Reward', episode_list[-1], "local", local, "offload", offload)
    #print('OBS', obs)
