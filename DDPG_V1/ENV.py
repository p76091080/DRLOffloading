...
class Environment:
    ...
    def step(self, actions, T_offload, T_computing):
        if T_offload != 0:
            ...
            threading.Thread(target=self.thread_radio, args=(
                T_offload, actions[1] * self.residual_radio)).start()  # target: 被執行的物件，由run()方法執行 args: target物件使用的引數
            threading.Thread(target=self.thread_computing, args=(
                T_computing, actions[2] * self.residual_computing)).start()
    ...
    def observed(self):
        ...
        # tau = random.uniform(0, 0.015)  # unknown
        tau = 0.015
        dis_n = random.uniform(10, 20)
        h = self.Ad*pow(3*1e8/(4*pi*self.fc*dis_n), 2)
        return np.array([x, y, d, c, tau, self.p*h, random.uniform(0.4*1e9, 0.6*1e9)])

    def show_reward(self, count=5):
